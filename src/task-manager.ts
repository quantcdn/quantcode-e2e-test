...
export class TaskManager {
  ...
  sortBy(field: "priority" | "createdAt" | "status"): Task[] {
    ...
    if (field === "priority") {
      ...
      return all.slice().sort((a, b) => order[a.priority] - order[b.priority])
    }
    if (field === "createdAt") {
      return all.slice().sort((a, b) => a.createdAt.getTime() - b.createdAt.getTime())
    }
    // status: alphabetical
    return all.slice().sort((a, b) => a.status.localeCompare(b.status))
  }
}
