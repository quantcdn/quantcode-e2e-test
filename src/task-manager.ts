...
export class TaskManager {
  ...
  sortBy(field: "priority" | "createdAt" | "status"): Task[] {
    ...
    return tasks.sort((a, b) => {
      switch (field) {
        case "priority":
          return priorityRank[a.priority] - priorityRank[b.priority] || this.byInsertion(a, b)
        case "status":
          return statusRank[a.status] - statusRank[b.status] || this.byInsertion(a, b)
        case "createdAt":
          return a.createdAt.getTime() - b.createdAt.getTime() || this.byInsertion(a, b)
      }
    })
  }

  private byInsertion(a: Task, b: Task): number {
