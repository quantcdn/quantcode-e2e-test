...
export class TaskManager {
  ...
  sortBy(field: "priority" | "createdAt" | "status"): Task[] {
    ...
    const tiebreak = (a: Task, b: Task) => Number(a.id) - Number(b.id)

    return Array.from(this.tasks.values()).sort((a, b) => {
      switch (field) {
        case "priority":
          return priorityOrder[a.priority] - priorityOrder[b.priority] || tiebreak(a, b)
        case "createdAt":
          return a.createdAt.getTime() - b.createdAt.getTime() || tiebreak(a, b)
        case "status":
